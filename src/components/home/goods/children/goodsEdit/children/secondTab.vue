	...
	<div class="currency">
		...
		<div class="z_info">
			...
			<div class="goods_number">
				...
				<!-- <el-input v-model="goods.numberGD1" :maxlength="40" size="medium" style="width: 150px;" placeholder="请填写数量" clearable>
				</el-input>
				<el-input v-model="goods.numberGD2" :maxlength="40" size="medium" style="width: 150px; margin-left:20px;" placeholder="请填写价格" clearable>
				</el-input>
				<el-button style="margin-left:20px" type="info" @click="addFind0" plain>+ 添加</el-button> -->
				<div class="add_input" v-for="(find0, index) in numData" :key="index">
...
export default {
	...
	methods: {
		addFind0: function () {
			this.numData.push({ number: '', order_price: '' });
		},
		btnClick1(val) {
			if(val) {
				// $(".formData").val(val)
		...
		getSpecItem(status, specres) {
			...
			this.$HTTP(this.$httpConfig.getGoodsSpec, { spec: spec, goods_id: this.goodsId }, "post")
			.then(res => {
				$(".g_editss").html("");
		...
		getSpecItem1() {
			...
			this.$httpConfig.getGoodsSpec,
				{ spec: this.spec_select, goods_id: this.goodsId },
				"post"
			)
			.then(res => {
        ...
        getData() {
			this.$HTTP(this.$httpConfig.getGoodsDetail,{id: this.$route.params.id}).then((res) => {
				this.numData = res.data.data.num
				this.goods = res.data.data.goods;
				this.goodsId = res.data.data.goods_detail.goods_id;
		...
		addGoods() {
			...
			this.goods['num'] = this.numData;
			this.goods['goods_id'] = this.numData[0].goods_id
			this.goods['item'] = inputList;
			// 提交商品基本信息
			this.$HTTP(this.$httpConfig.saveGoods, this.goods, 'post').then((res) => {
				...
				// this.$emit('selectTab');
				this.$emit('imageList');
				console.log(111111, this.$emit('imageList'))
