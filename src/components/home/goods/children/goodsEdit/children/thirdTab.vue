                ...
                <div class="img-list">
                    ...
                    <div class="img-content" v-for="(item,key) in fileList" :key="key">
                        <div class="activeinneruploadDiv">
                            ...
                            <div class="activeunderimg" v-if="item.name !== null && key == 0">
                                <img class="updatecss" src="../../../../../../assets/images/thirdTab/2.png">
                                <span>默认主图</span>
                            </div>
                        </div>  
...
export default {
    ...
    methods: {
            ...
            handleResponse(response) {
                ...
                return "https://admin.yisample.com/" + response.data;
            },
            // videoList() {
			// 	this.$HTTP(this.$httpConfig.getVideoList, {goods_id: this.$route.params.id}).then((res) => {
			...
			UploadImage(param) {
				this.$HTTP(this.$httpConfig.getGoodsImgCofig, { goods_id: this.$route.params.id}, 'post').then(res => {
					let data = res.data.data;
					...
					this.axios.post(this.$store.state.image_api_url + "FileUpload/uploadImageToLocal/", formData).then((res) => {
						if (res.data.status == 0) {
                            ...
                            // this.fileList[this.fileList.length - 1].url = this.URL + res.data.data   //original
            ...
            // 添加图片
			addPicture() {
				// this.$HTTP(this.$httpConfig.saveGoodsImg, {pic_url: this.uploadFile,video:this.video}).then((res) => {
                    this.$HTTP(this.$httpConfig.saveGoodsImg, {pic_url: this.uploadFile, goods_id: this.$route.params.id}).then((res) => {
					this.$message.success(res.data.message);
                    Toast({
                        message: res.data.message,
                        position: 'middle'
                    });
                    this.$emit('thirdToFour', 3)
                    this.$emit('selectTab');
