...
export default {
    ...
    methods: {
            ...
            imageList() {
				this.$HTTP(this.$httpConfig.getGoodsImgList, {goods_id: ""}).then((res) => {
					if (!res.data.data) {
						return false
					}
			...
			UploadImage(param) {
				this.$HTTP(this.$httpConfig.getGoodsImgCofig, {goods_id: ""}, 'post').then(res => {
					let data = res.data.data;
			...
			addPicture() {
				// this.$HTTP(this.$httpConfig.addGoodsPic, {pic_url: this.uploadFile,video:this.video}).then((res) => {
                    this.$HTTP(this.$httpConfig.addGoodsPic, {pic_url: this.uploadFile, goods_id: ""}).then((res) => {
					this.$message.success(res.data.message);
                    Toast({
                        message: res.data.message,
                        position: 'middle'
