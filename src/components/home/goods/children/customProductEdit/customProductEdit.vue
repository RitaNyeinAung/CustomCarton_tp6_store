      ...
      <el-tabs type="card" @tab-click="handleClick" v-model="changeTab">
        <el-tab-pane name="通用信息" label="通用信息">
          <div class="customize_details">
            ...
            <!-- <p>制造尺寸</p> -->
            <p></p>
            <div v-for="(item, index) in spaceTemplate" :key="index" style="display: flex; ">
              <p class="leftp" style=""><span>*</span>{{item.name}}：</p>
              <div class="rightinput">
                  <el-input style="display: none;" v-model="checkInputID[index]" disabled></el-input>
                  <div class="add_input" v-for="(find0, ind) in item.item" :key="ind">
                    <el-input :style="(item.type == 'long' || item.type == 'wide' || item.type == 'high' || item.type == 'chief' || item.type == 'width' || item.type == 'thick') ? 'display: none' : '' " class="loopinput" v-model="find0.item" placeholder="规格参数"></el-input>
            ...
            <button @click="editCustomInfo" class="next_step">确认提交</button>
          </div>
        </el-tab-pane>
        <el-tab-pane name="商品相册" label="商品相册">
          ...
          <button @click="editPicture" class="confirm_submit">确认提交</button>
        </el-tab-pane>
      </el-tabs>
    </div>
  </div>
...
export default {
  ...
  methods: {
    getCustomData() {
        this.$HTTP(this.$httpConfig.customizedDetail, { id: this.$route.params.id }, "post" )
        .then(res => {
            let list = res.data.data;
            this.title = res.data.data.title;
            this.number = res.data.data.number;

            this.value = res.data.data.one_name
            this.value1 = res.data.data.two_name
            this.value2 = res.data.data.three_name
            this.tid = res.data.data.t_id;
            this.spaceTemplate = res.data.data.space

            this.checkInputID = this.spaceTemplate.map(item => {
              return item.id
            })
            this.firIdEdit = res.data.data.class_id
            this.secondId = res.data.data.class_two
            this.thirdId = res.data.data.class_three
    ...
    addInputMethod(val) {
        let tempArr = [...this.spaceTemplate]
        tempArr.forEach((item, data) => {
            if(item.id == val) {
                this.spaceTemplate[data].item.push({ item: '', price: '' });
                return;
            }
        })
        let sss = [...this.spaceTemplate]
        this.spaceTemplate = []
        this.spaceTemplate = sss
    ...
    getData() {
        ...
        .then(res => {
          ...
          this.$HTTP(this.$httpConfig.getClassById, {
            goods_class_id: this.secondId
          })
          .then(res => {
            this.secondData = res.data.data;
            this.$HTTP(this.$httpConfig.getClassById, {
              goods_class_id: this.thirdId
            })
            .then(res => {
    ...
    getCustomTemplateList() {
        ...
        .then(res => {
            ...
            for(let i = 0; i< this.templateList.length; i++) {
              if(this.templateList[i].id == this.tid) {
                this.value3 = this.templateList[i].name;
              }
            }
        })
        .catch(err => {
    ...
    getcustomTemplateDetail(id3) {
        ...
        .then(res => {
            ...
            this.spaceTemplate = res.data.data;
            let tempLen  = this.spaceTemplate.length
            this.nameArr = []
            this.checkInputID = []
            for(let i = 0; i < tempLen; i++) {
                this.checkInputID.push(this.spaceTemplate[i].id)
                this.spaceTemplate[i]['item'] = [{ item: '', price: '' }]
                this.nameValue = this.spaceTemplate[i].item
            }
            this.nameArr = this.spaceTemplate
        })
        .catch(err => {
          this.nameArr = []
            this.checkInputID = []
    ...
    editCustomInfo() {
      ...
      for(let i = 0; i < tempLen; i++) {
          valueInput.push([{item: ''}])
          priceInput.push([{price: ''}])
          
          let len = this.spaceTemplate[i].item
          for(let j = 0; j < len.length; j++) {
              if(len[j].item !== null && len[j].item !== "") {
                  let tempval = len[j].item
                  valueInput[i][j] = tempval
              } else if(this.spaceTemplate[i].type == 'long' || this.spaceTemplate[i].type == 'wide' || this.spaceTemplate[i].type == 'high' || 
                this.spaceTemplate[i].type == 'chief' || this.spaceTemplate[i].type == 'width' || this.spaceTemplate[i].type == 'price') {
                  valueInput[i][j] = "mm"
              } else {
                  valueInput[i][j] = ""
              }
      ...
      if(!(typeof this.value === 'string' || this.value instanceof String) && !(typeof this.value1 === 'string' || this.value1 instanceof String)
        && !(typeof this.value === 'string' || this.value instanceof String)) {
          this.$HTTP(this.$httpConfig.customizedModify, {
            title: this.title,
            number: this.number,
          ...
          this.$HTTP(this.$httpConfig.customizedModify, {
            title: this.title,
            class_one: this.firIdEdit,
            class_two: this.secondId,
            class_three: this.thirdId,
    ...
    editPicture() {
        this.$HTTP(this.$httpConfig.saveGoodsImg, {pic_url: this.uploadFile, goods_id: this.goodsId}).then((res) => {
            this.$message.success(res.data.message);
        });
		}
  }
