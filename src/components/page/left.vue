...
export default {
    ...
    mounted() {
        ...
        if (this.$route.query.type == "customProduct") {
            for (var i = 0; i < this.status.length; i++) {
                this.$set(this.status, i, false);
            }
            this.$set(this.status, 6, true);
        }
        if (this.$route.query.type == "customProductList") {
            for (var i = 0; i < this.status.length; i++) {
                this.$set(this.status, i, false);
            }
            this.$set(this.status, 7, true);
        }
        ...
        if(this.$route.query.type == 'inquiryPurchase') {
            for (var i = 0; i < this.status.length; i++) {
                this.$set(this.status, i, false);
            }
            this.$set(this.status, 1, true);
        }
      if(this.$route.query.type == 'goodsManage') {
        for (var i = 0; i < this.status.length; i++) {
          this.$set(this.status, i, false);
        }
        this.$set(this.status, 1, true);
      }
      if(this.$route.query.type == 'askQuestion') {
        for (var i = 0; i < this.status.length; i++) {
          this.$set(this.status, i, false);
        }
        this.$set(this.status, 2, true);
      }
      if(this.$route.query.type == 'invoice') {
        for (var i = 0; i < this.status.length; i++) {
          this.$set(this.status, i, false);
        }
        this.$set(this.status, 1, true);
      }
      if(this.$route.query.type == 'evaluate') {
        for (var i = 0; i < this.status.length; i++) {
          this.$set(this.status, i, false);
        }
        this.$set(this.status, 3, true);
      }
      if(this.$route.query.type == 'refund') {
        for (var i = 0; i < this.status.length; i++) {
          this.$set(this.status, i, false);
        }
        this.$set(this.status, 2, true);
