<template>
    <div>
        ...
        <el-form-item :label="listTitle">
            <el-table v-if="updateGoods[0].data" :data="updateGoods[0].data" border style="width: 70%;">
                <el-table-column prop="id" label="ID">
                </el-table-column>
                <el-table-column show-overflow-tooltip prop="title" label="ddd商品名称">
                </el-table-column>
        ...
        <el-form-item :label="listTitle1">
            <el-table v-if="updateGoodsChild" :data="updateGoodsChild" border style="width: 70%;">
                <el-table-column prop="id" label="ID">
                </el-table-column>
    ...
    watch: {
        ...
        sendUpdateData() {
            if(this.updateAgain == true) {
                ...
                if(this.sendUpdateData == '') {
                    this.updateGoods = []
                } else if(this.sendUpdateData && this.sendUpdateData[0] && this.sendUpdateData[0].clearData && this.sendUpdateData[0].clearData == true) {
                    this.hasCityData = []
                ...
                if(this.status) {
                    this.updateGoodsChild = this.updateGoods[0].data[0].GoodsSpec
        ...
        checkList() {
            if (this.status) {
                let tempArr = []
                let tempList = []
                if(this.updateAgain == true || this.promotion == true) {
                    // alert(JSON.stringify(this.checkList) + ' If checklist ' + 
                    // JSON.stringify(this.updateGoods[0].data[0]))
                    for(let j = 0; j < this.checkList.length; j++) {
                        for(let i = 0; i < this.updateGoodsChild.length; i++) {
                            if(this.checkList[j] == this.updateGoodsChild[i].item) {
                                ...
                                this.updateGoods[0].data[0].GoodsSpec[i].isAdded = true // Edit
                                this.updateGoodsChild[i].isAdded = true // Edit 
                            }
                        }
                    }
                    let groupId = tempArr;
                    ...
                    if(this.updateAgain == true) {
                        let updateAgain = 'updAgain'
                        this.sendDataUp(this.updateGoods, this.updateAgain, updateAgain);
                    } else {
                        this.sendDataUp(this.updateGoods, this.promotion);
                    }
                } else {
                    // alert(JSON.stringify(this.checkList) + ' else checklist ' + JSON.stringify(this.updateGoods[0].data[0])) // I think this block not work and unneed
    ...
    methods: {
        loopsendUpdate() {
            if(this.updateAgain == true) {
                ...
                this.updateGoodsChild = []
                if(ss.length > 0) {
                    this.updateGoodsChild = ss[0].GoodsSpec
                    for(let i = 0; i < this.updateGoodsChild.length; i++) {
                        if(this.updateGoodsChild[i].isAdded == true) {
                            this.checkList.push(this.updateGoodsChild[i].item)
                        }
        ...
        submitSelect() {
            ...
            if (this.status) {
                ...
                let ss = this.updateGoods[0].updateLoopCount
                this.updateGoods = []
                let dd = []
                dd.push(this.multipleSelection)
                this.updateGoods.push({updateLoopCount: ss, data: dd});
                this.updateGoodsChild = []
                for(let i = 0; i < this.updateGoods[0].data[0].GoodsSpec.length; i++) {
                    this.updateGoodsChild.push(this.updateGoods[0].data[0].GoodsSpec[i]);
                }
        ...
        // 发送数据给父组件
        sendDataUp(data, updateAgain, updAga) {
            this.$emit('sendGoodsData', this.hasCityId, data, updateAgain, updAga);
        }
    },
}
</script>
