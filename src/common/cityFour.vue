...
export default {
    ...
    methods: {
        // 获取省
        getClass() {
            this.$HTTP(this.$httpConfig.getRegionArea,{parent_id: 0}).then((res) => {
                if (!res.data.data) {
                    this.$layer.msg('暂无数据:(');
                    return;
                }
        ...
        selectChild() {
            ...
            this.$HTTP(this.$httpConfig.getRegionArea,{parent_id: parseInt(this.params.prov)}).then((res) => {
                if (!res.data.data) {
        ...
        selectChildByTwo() {
            ...
            this.$HTTP(this.$httpConfig.getRegionArea,{parent_id: parseInt(this.params.city)}).then((res) => {
                if (!res.data.data) {
                    this.$layer.msg('暂无数据:(');
                    return;
                }
        ...
        selectChild2() {
            if(Number(this.params.prov)){
                this.$HTTP(this.$httpConfig.getRegionArea,{parent_id: Number(this.params.prov)}).then((res) => {
                if (!res.data.data) {
                    this.$layer.msg(res.data.message);
                    return;
                }
        ...
        // 发送数据给父组件
        sendData() {
            this.$emit('sendcityData1', this.params);
        }
    },
}
</script>